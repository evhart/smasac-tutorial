...
RUN apk add --no-cache git python3 python3-dev python2 python2-dev py2-numpy py2-numpy-f2py py2-scipy py3-numpy py3-numpy-f2py py3-scipy py-numpy-dev gfortran gcc build-base wget freetype-dev libpng-dev openblas-dev cython cython-dev geos@testing geos-dev@testing gdal@testing gdal-dev@testing && \
    python3 -m ensurepip && \
    python2 -m ensurepip && \
    rm -r /usr/lib/python*/ensurepip && \
    python3 -m pip install --upgrade pip==9.0.3 setuptools && \
    python2 -m pip install --upgrade pip==9.0.3 setuptools && \
    ln -sf pip3 /usr/bin/pip && \
    ln -sf python3 /usr/bin/python && \
    rm -r /root/.cache

# Install the requirements:
COPY requirements2.txt /home/requirements2.txt
COPY requirements3.txt /home/requirements3.txt
RUN python3 -m pip install --no-cache -r requirements3.txt
RUN python2 -m pip install --no-cache -r requirements2.txt
